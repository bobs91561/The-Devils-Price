...
#define VOLUMETRIC_CLOUD_SHADOW_DITHER 0.005
...
#define VOLUMETRIC_CLOUD_SHADOW_FADE 1.75
...
float ComputeCloudShadowStrength(float3 worldPos, uint dirIndex)
{
	...
	else
	{
			...
			_WeatherMakerDirLightVar1[dirIndex].y == 0.0 && _WeatherMakerDirLightColor[dirIndex].a > 0.001)
		{
			...
			float3 cloudRayDir = normalize(float3(rayDir.x, rayDir.y + _CloudRayOffsetVolumetric, rayDir.z));
			float distanceFactor = distance(worldPos, _WorldSpaceCameraPos) * VOLUMETRIC_CLOUD_SHADOW_FADE;
			float fade = 1.0 - min(1.0, _ProjectionParams.w * distanceFactor);
...
#define VOLUMETRIC_CLOUD_SAMPLE_SHADOW \
				marchPos += marchDir; \
				heightFrac = GetCloudHeightFractionForPoint(marchPos); \
				weatherData = CloudVolumetricSampleWeather(marchPos, 0.0); \
				cloudType = CloudVolumetricGetCloudType(weatherData); \
				samp = (randomDither * CloudVolumetricGetCoverage(weatherData) * GetDensityHeightGradientForHeight(heightFrac, cloudType)); \
				samp *= samp; \
				cloudCoverage += (samp * samp);

				UNITY_LOOP
				for (uint i = 0; i < _CloudVolumetricShadowSampleCount && cloudCoverage < 0.99; i++)
				{
					VOLUMETRIC_CLOUD_SAMPLE_SHADOW;
				}

#undef VOLUMETRIC_CLOUD_SAMPLE_SHADOW
				...
				cloudCoverage = min(_WeatherMakerCloudVolumetricShadow, (1.0 - saturate(cloudCoverage * fade * _WeatherMakerDirLightPower[dirIndex].z)));
