Shader "NatureManufacture Shaders/Lava/Standard Specular Lava CutOut"
{
	Properties
	{
		...
		_SpecularRGBEmissionA("Specular (RGB) Emission (A)", 2D) = "white" {}
	...
	SubShader
	{
		...
		uniform float4 _CutOutMaskA_ST;
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			half4 blendOpDest189 = ( _DetailAlbedoPower * tex2D( _DetailAlbedoMap, uv_DetailAlbedoMap ) );
			...
			half4 tex2DNode29 = tex2D( _SpecularRGBEmissionA, uv_MainTex );
			...
			float2 uv_LavaNoise = i.uv_texcoord * _LavaNoise_ST.xy + _LavaNoise_ST.zw;
			float2 panner233 = ( _SinTime.x * ( _LavaNoiseSpeed * float2( -1.2,-0.9 ) ) + uv_LavaNoise);
			float2 panner235 = ( _SinTime.x * _LavaNoiseSpeed + uv_LavaNoise);
