Shader "NatureManufacture Shaders/Lava/Standard Specular UV Free Lava"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 uv_NoiseR = i.uv_texcoord * _NoiseR_ST.xy + _NoiseR_ST.zw;
			float2 panner3803 = ( _SinTime.x * ( _LavaNoiseSpeed * float2( -1.2,-0.9 ) ) + uv_NoiseR);
			float2 panner3804 = ( _SinTime.x * _LavaNoiseSpeed + uv_NoiseR);
