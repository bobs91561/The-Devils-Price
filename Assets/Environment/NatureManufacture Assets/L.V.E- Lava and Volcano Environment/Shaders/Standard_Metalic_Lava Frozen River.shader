Shader "NatureManufacture Shaders/Lava/Standard Metalic Lava Frozen River"
{
	Properties
	{
		...
		_MetalicRAmbientOcclusionGHeightBEmissionA("Metalic (R) Ambient Occlusion (G) Height (B) Emission(A)", 2D) = "white" {}
		...
		_CoverMetalicRAmbientocclusionGHeightBEmissionA("Cover Metalic (R) Ambient occlusion (G) Height (B) Emission (A)", 2D) = "white" {}
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v )
		{
			...
			half4 tex2DNode2 = tex2Dlod( _MetalicRAmbientOcclusionGHeightBEmissionA, half4( uv_MainTex, 0, 1.0) );
			float temp_output_580_0 = ( tex2DNode2.b * _TesselationDepth );
			float2 uv_TextureSample3 = v.texcoord.xy * _TextureSample3_ST.xy + _TextureSample3_ST.zw;
			half4 tex2DNode536 = tex2Dlod( _CoverMetalicRAmbientocclusionGHeightBEmissionA, half4( uv_TextureSample3, 0, 1.0) );
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult570 = lerp( lerpResult479 , UnpackScaleNormal( tex2D( _CoverNormal, uv_TextureSample3 ), _CoverNormalScale ) , _CoverNormalBlend);
			half4 tex2DNode536 = tex2D( _CoverMetalicRAmbientocclusionGHeightBEmissionA, uv_TextureSample3 );
			half4 tex2DNode2 = tex2D( _MetalicRAmbientOcclusionGHeightBEmissionA, uv_MainTex );
			float temp_output_578_0 = ( ( pow( tex2DNode536.b , _CoverHeightmapContrast ) * _CoverHeightmapDepth ) + ( tex2DNode2.b * _HeightmapDepth ) );
			float clampResult579 = clamp( temp_output_578_0 , 0.0 , temp_output_578_0 );
			float clampResult587 = clamp( clampResult579 , 0.0 , 1.0 );
			float temp_output_586_0 = ( 1.0 - clampResult587 );
			...
			half4 tex2DNode537 = tex2D( _TextureSample3, uv_TextureSample3 );
